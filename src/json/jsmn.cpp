  ...
  for (; parser->pos < len && js[parser->pos] != '\0'; parser->pos++)
  {
    ...
    case '[':
      ...
      if (parser->toksuper != -1)
      {
        ...
        if (skipDepth > 0)
        {
          // We already are skipping: do not signal and go a level deeper
          skipDepth++;
        }
        else
        {
          // We are beginning an object as a key value, signal it
          fillAccept(&acceptArg,
                     (token->type == JSMN_OBJECT) ? JSMN_Accept_ObjectBegin : JSMN_Accept_ArrayBegin,
                     js, parser->toksuper, parentToken);
    ...
    case ']':
      ...
      // Start from the last token and navigate the hierarchy upwards looking for the object or array we need to close
      token = &tokens[parser->toknext - 1];
      for (;;)
      {
        if (token->start != -1 && token->end == -1)
        {
          // Found a token that has a start but not an end, it must be the object or array we are closing
          if (token->type != type)
          {
            return JSMN_ERROR_INVAL;
          }
          // Set its end and navigate up one level
          ...
          if (parser->toksuper != -1)
          {
            ...
            else 
            {
              jsmntok_t *parentToken = &tokens[parser->toksuper];
              fillAccept(&acceptArg,
                        (token->type == JSMN_OBJECT) ? JSMN_Accept_ObjectEnd : JSMN_Accept_ArrayEnd,
                        js, parser->toksuper, parentToken);
        ...
        if (token->parent == -1)
        {
          // We reached the root without finding one, how can this happen?
          break;
        }
        // Go up one level, the current token must be a value
