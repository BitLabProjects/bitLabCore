...
SSD1306OverI2C::SSD1306OverI2C(I2C &i2c, PinName RST, uint8_t i2cAddress, uint8_t rawHeight, uint8_t rawWidth)
    : SSD1306(RST, rawHeight, rawWidth), mi2c(i2c), mi2cAddress(i2cAddress)
{
  begin();
  splash();
  display();
...
void SSD1306OverI2C::command(uint8_t c)
{
  char buff[2];
...
void SSD1306OverI2C::data(uint8_t c)
{
  char buff[2];
...
void SSD1306OverI2C::sendDisplayBuffer()
{
  char buff[17];
  buff[0] = 0x40; // Data Mode
...
void SSD1306::splash(void)
{
  ...
  std::copy(
      &adaFruitLogo[0], &adaFruitLogo[0] + (_rawHeight == 32 ? sizeof(adaFruitLogo) / 2 : sizeof(adaFruitLogo)), buffer.begin());
#endif
}
